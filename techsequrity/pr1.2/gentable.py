...
incident = {
    "Дата інциденту": "Початок 2026 р.; оприлюднено у квітні 2026 р.",
    "Постраждала організація": (
        "Державні установи країн Південно-Східної Азії"
    ),
    "Кількість постраждалих": (
        "Не розкрито; кілька урядових організацій"
    ),
    "Що було зроблено?": (
        "Китайсько-орієнтовані хакерські групи використали zero-day у TrueConf "
        "для компрометації урядових мереж у межах кампанії TrueChaos. "
        "Атака дозволяла розгортати шкідливе ПЗ через механізм оновлення клієнта."
    ),
    "Які експлойти використовувалися?": (
        "CVE-2026-3502 — відсутність перевірки цілісності оновлень у TrueConf, "
        "що дозволяє доставляти шкідливі файли; використання Havoc C2, DLL side-loading."
    ),
    "Яким чином налагоджено захист?": (
        "Оновлення до TrueConf 8.5.3 або новіших версій, контроль серверів оновлень, "
        "моніторинг мережевого трафіку, використання EDR, сегментація мережі "
        "та перевірка цілісності програмного забезпечення."
    ),
    "Посилання": "https://www.scworld.com/brief/attacks-with-trueconf-zero-day-hit-southeast-asian-governments"
}
...
# Збереження
doc.save("Звіт_TrueConf_ZeroDay_TrueChaos.docx")
...
incident = {
    "Дата інциденту": "Жовтень 2025 р. (розкрито у 2026 р.)",
    "Постраждала організація": "Wynn Resorts (готельно-казино оператор, США)",
    "Кількість постраждалих": "Понад 21 000 співробітників (≈21 775 осіб)",
    "Що було зроблено?": (
        "Зловмисники отримали доступ до внутрішніх HR-систем компанії "
        "та викрали персональні дані співробітників. Після вимоги викупу "
        "дані були видалені (ймовірно після сплати). Компанія повідомила "
        "постраждалих та запропонувала послуги моніторингу кредитної історії."
    ),
    "Які експлойти використовувалися?": (
        "Атака, пов’язана з угрупованням ShinyHunters; компрометація "
        "внутрішніх систем і облікових записів, доступ до HR-даних. "
        "Ймовірно використано техніки викрадення облікових даних та lateral movement."
    ),
    "Яким чином налагоджено захист?": (
        "Розслідування інциденту, залучення зовнішніх експертів з кібербезпеки, "
        "посилення контролю доступу, моніторинг систем, повідомлення постраждалих, "
        "запровадження сервісів захисту від викрадення особистих даних."
    ),
    "Посилання": "https://www.scworld.com/brief/over-21k-wynn-resorts-employees-affected-by-breach"
}
...
# Збереження файлу
doc.save("Звіт_Wynn_Resorts_Data_Breach.docx")
