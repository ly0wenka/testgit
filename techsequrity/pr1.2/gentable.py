...
# Створення документа
doc = Document()
doc.add_heading('Звіт про кіберінцидент / політику (SC Media)', level=1)

incident = {
    "Дата інциденту": "Квітень 2026 р.",
    "Постраждала організація": (
        "Користувачі інтернет-сервісів у США (законодавча ініціатива House GOP)"
    ),
    "Кількість постраждалих": (
        "Потенційно всі громадяни США (масштаб — національний рівень)"
    ),
    "Що було зроблено?": (
        "Республіканці Палати представників США представили законопроєкт "
        "Secure Data Act, спрямований на створення єдиного національного "
        "стандарту захисту персональних даних та регулювання їх збору і використання."
    ),
    "Які ризики / проблеми розглядаються?": (
        "Надмірний збір персональних даних, використання їх для таргетованої реклами, "
        "передача третім сторонам та іноземним урядам, відсутність прозорості у використанні даних."
    ),
    "Які механізми / регулювання пропонуються?": (
        "Право користувачів відмовитися (opt-out) від збору даних, "
        "обов’язкове інформування про збір і використання інформації, "
        "портативність даних, згода батьків на обробку даних дітей, "
        "посилений контроль над data brokers через FTC." 
    ),
    "Яким чином налагоджено захист?": (
        "Встановлення федеральних стандартів конфіденційності, "
        "обмеження збору даних до необхідного мінімуму, "
        "підвищення прозорості, реєстрація брокерів даних, "
        "контроль і аудит використання персональної інформації."
    ),
    "Проблеми та критика": (
        "Критики зазначають, що закон може бути слабшим за існуючі закони окремих штатів, "
        "скасовує суворіші місцеві норми, не дозволяє громадянам подавати позови, "
        "та дає компаніям час на виправлення порушень без штрафів."
    ),
    "Посилання": "https://www.scworld.com/brief/house-gop-eyes-nationwide-rules-on-data-collection"
}
