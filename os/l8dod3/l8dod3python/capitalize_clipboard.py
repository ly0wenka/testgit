...
# --- Clipboard Functions ---
# ---------------------------
def sentence_case_clipboard():
    try:
        text = pyperclip.paste()
        # .capitalize() робить першу літеру великою, а решту — малими
        pyperclip.copy(text.capitalize())
        print("Clipboard converted to Sentence case!")
    except Exception as e:
        print(f"Error: {e}")

def normalize_spaces_clipboard():
    try:
        text = pyperclip.paste()
        ...
        print("Clipboard normalized: enters -> space, multiple spaces -> one")
    except Exception as e:
        print(f"Error: {e}")
...
icon = pystray.Icon(
    ...
    menu=pystray.Menu(
        pystray.MenuItem("Sentence Case", lambda icon, item: sentence_case_clipboard()),
        pystray.MenuItem("Normalize Spaces", lambda icon, item: normalize_spaces_clipboard()),
...
# ---------------------------
def setup_hotkeys():
    keyboard.add_hotkey("ctrl+shift+i", sentence_case_clipboard())
    keyboard.add_hotkey("ctrl+shift+space", normalize_spaces_clipboard)
