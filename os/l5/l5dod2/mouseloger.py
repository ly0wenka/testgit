...
def log_mouse(event_type, x, y, button=None):
    with open(mouse_log_file, "a", encoding="utf-8") as f:
        if button:
            f.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')} - {event_type} - Button: {button} at ({x},{y})\n")
        else:
            f.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')} - {event_type} at ({x},{y})\n")
...
def mouse_event_handler(event):
    if isinstance(event, mouse.MoveEvent):
        log_mouse("Move", event.x, event.y)
    elif isinstance(event, mouse.ButtonEvent):
        action = "Pressed" if event.event_type == "down" else "Released"
        x, y = mouse.get_position()  # отримуємо актуальні координати
        log_mouse(action, x, y, event.button)
